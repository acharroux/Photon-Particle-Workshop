...
void SoftwarePanel::initAfterConnect(void)
{
    ...
    {
        // Strangely, getting back the device name and IP are quite tricky
...
void SoftwarePanel::display(void)
{
    ...
    setFont(LargeFont);
    getDisplay().printf("%u-%u", mSoftwareState.getProductID(), mSoftwareState.getProductVersion());
    newLine();
...
String SoftwarePanel::JSONDump(const char *data) const
{
    ...
    return String::format(
        "{ \"Name\": \"%s\", \"uptime\": \"%s\", \"NbRanges\": %lu, \"NbValidRanges\":%lu \"PRODUCT_ID\": %u, \"PRODUCT_VERSION\": %u}",
        mDeviceName.c_str(),
        uptime().c_str(),
        mSoftwareState.getNbRanges(),
        mSoftwareState.getNbValidRanges(),
        mSoftwareState.getProductID(),
        mSoftwareState.getProductVersion());
}
